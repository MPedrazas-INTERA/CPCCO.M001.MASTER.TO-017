...
def generate_map1(arr, ifile, ofile, ptitle, levels, colors, xy, show_well):
    ...
    if show_well:
        ...
        # [1] Monitoring wells -----------------------------------------
        ifile_zone = f'input/shp_files/100D/monitoring_wells.shp'
        zones = gpd.read_file(ifile_zone)
        zones.plot(ax = ax, color = 'black', label = 'Monitoring Well', markersize = 9, zorder=2.5)
        
        # Add text labels next to each well
        for idx, row in zones.iterrows():
            ax.annotate(row['ShortName'],
                        (row.geometry.x, row.geometry.y),
                        textcoords="offset points",
                        xytext=(2,2),  # Adjust the position of the text label
                        ha='left', fontsize=6)
        
        # [2] Aquifer Tubes -----------------------------------------
        ifile_zone = f'input/shp_files/100D/aquifer_tubes.shp'
        zones = gpd.read_file(ifile_zone)
        zones.plot(ax = ax, color = 'blue', label = 'Aquifer Tube', markersize = 9, zorder=2.5)
        ...
        for idx, row in zones.iterrows():
            ax.annotate(row['ShortName'],
                        ...
                        ha='left', fontsize=6)        
        # [3] Extraction wells -----------------------------------------
        ifile_zone = f'input/shp_files/100D/extraction_wells.shp'
        zones = gpd.read_file(ifile_zone)
        zones.plot(ax = ax, color = 'black', label = 'Extraction Well', markersize = 9,
                   marker='^', edgecolor='black', zorder=2.5)
        ...
        # [4] Injection wells -----------------------------------------
        ifile_zone = f'input/shp_files/100D/injection_wells.shp'
        zones = gpd.read_file(ifile_zone)
        zones.plot(ax = ax, color = 'black', label = 'Injection Well', markersize = 9,
                   marker='v', edgecolor='black', zorder=2.5)
    ...
    if show_cs:
        ...
        zones = gpd.read_file(ifile_zone)
        # Keep track of labeled sources
        labeled_sources = set()
        for idx, row in zones.iterrows():
            # Check if the source has been labeled before
            if (row['Waste_Site'] not in labeled_sources) and (row['Waste_Site'] in ['183-H SEB', '100-H-46', '107-H RB']):
                # Add text labels to continuing sources
                ax.annotate(row['Waste_Site'],  
                            (row.geometry.centroid.x, row.geometry.centroid.y),
                            textcoords="offset points",
                            xytext=(5, -20),  # Adjust the position of the text label
                            ha='center', fontsize=6, fontweight="bold")
                labeled_sources.add(row['Waste_Site']) # Mark the source as labeled
        #print(zones['Waste_Site'].unique())
    ...
    if show_river:
        ...
        zone2.plot(color='#a6cee3', ax=ax, zorder=15,
                    alpha=0.5, linewidth=0.1, edgecolor='#a6cee3', label = 'River')
        #zone2.plot(ax=ax)
    ...
    #Replace title by figure tag:
    #ax.set_title(ptitle, fontsize = 8, color='darkgrey')
    title_location = (0.97, 0.01)  # Location of the title in figure coordinates
    ax.annotate(ptitle, xy=(title_location[0], title_location[1]), xycoords="axes fraction",
    ...
    # Remove axis labels, tick marks, and tick mark labels
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    ...
    # Add a scale bar
    scale_bar_length = 100  # Length of the scale bar in meters
    scale_bar_location = (0.05, 0.075)  # Location of the scale bar in figure coordinates
    scale_bar_color = 'black'

    # Calculate the position of the scale bar in data coordinates
    scale_bar_position = (ax.get_xlim()[0] + scale_bar_location[0] * (ax.get_xlim()[1] - ax.get_xlim()[0]),
                          ax.get_ylim()[0] + scale_bar_location[1] * (ax.get_ylim()[1] - ax.get_ylim()[0]))

    # Calculate the length of the scale bar in data coordinates
    scale_bar_length_data = scale_bar_length / (ax.get_xlim()[1] - ax.get_xlim()[0]) * (ax.get_ylim()[1] - ax.get_ylim()[0])

    # Plot the scale bar
    ax.plot([scale_bar_position[0], scale_bar_position[0] + scale_bar_length_data],
            [scale_bar_position[1], scale_bar_position[1]],
            color=scale_bar_color, linewidth=2)

    # Add a text label for the scale bar
    ax.text(scale_bar_position[0] + 0.5 * scale_bar_length_data, scale_bar_position[1] - 0.04 * (ax.get_ylim()[1] - ax.get_ylim()[0]),
            f'{scale_bar_length} m', color=scale_bar_color, ha='center', fontsize=8)
