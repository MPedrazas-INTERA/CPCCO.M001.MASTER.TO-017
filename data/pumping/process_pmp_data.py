...
def gen_pred_wellrate_csv_file(dfpred,npred_sp_with_pmp,ofile):
    ...
    for i in range(nmonths-1, npred_sp_with_pmp+1,1):
...
if __name__ == "__main__":
    ...
    # ifile_wname = os.path.join('..', 'received_030723', 'P&T_Well_to_PLC-ID.csv')
    ...
    ifile_wname = os.path.join(wdir, 'data', 'pumping', 'P&T_Well_to_PLC-ID.xlsx')
    wnames = pd.read_excel(ifile_wname)
    ...
    #sys = 'DX'
    plot_vol=False
    dx,nmonths = process_pmp(wdir,'DX', list_yr,plot_vol)
    hx,nmonths = process_pmp(wdir,'HX', list_yr,plot_vol)
    #hx=hx.iloc[1:]
    ...
    # Get dataframe for a given yeat
    ...
    # 
    col1 = ['NAME2','NAME','HMI','Type','System', 'XCOORDS', 'YCOORDS', 'ZCOORDS', 'Active', 'Sum']
    df=df[col1+col_name]
    # Save for checking
    ...
    #dfrate_2022_2125=gen_pred_wellrate_csv_file(dfpred,npred_sp_with_pmp,ofile)  
    ##add_future_rates(dfrate_2022_2125)
    ...
    #gen_well_info_csv_file(dfrate_2022_2125, ofile)
