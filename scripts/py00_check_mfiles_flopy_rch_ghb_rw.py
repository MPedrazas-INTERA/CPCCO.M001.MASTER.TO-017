...
from flopy.utils.postprocessing import (
    ...
    # get_saturated_thickness,  ## no longer available in 3.3.6 or greater
    get_water_table,
    get_gradients)
...
def arr2shp(arr,gridShp,ofile):
    ...
    try:
        ...
        gridShp = os.path.join(os.path.dirname(os.getcwd()), 'gis', 'shp', 'grid_with_centroids.shp')
...
def read_flow_files(nam_file, flow_model_ws):
...
def procees_rch(ml, odir):
    nlay, nr, nc, nper = ml.dis.nlay, ml.dis.nrow, ml.dis.ncol, ml.dis.nper
    rch = ml.rch.rech.array
    rch.shape
    coef_conv = 1000*365.25
    for sp in range(100,101+1, 1):
        # sp=1
        ...
        ofile = os.path.join(odir, f'check_rch_sp_{sp}.png')
        fig.savefig(ofile, dpi=300, transparent=False, bbox_inches='tight')
        # save to shp
        gridShp = os.path.join(os.path.dirname(os.getcwd()), 'gis', 'shp', 'grid_with_centroids.shp')
        # ofile_shp = f'output/ml_checking/rch/shp/check_rch_sp{sp}.shp'
        ofile_shp = os.path.join(odir, f'check_rch_sp{sp}.shp')
    ...
    df_rch_ts.plot()
    df_rch_ts.to_csv(os.path.join(odir, f'rch_all_ts_row{ir}_col{ic}.csv'))
    print('Done processing recharge!!!')

def process_riv(ml, odir):
    nlay, nr, nc, nper = ml.dis.nlay, ml.dis.nrow, ml.dis.ncol, ml.dis.nper
    ...
    for sp in range(len(riv)):
        stage.append(riv[0]['stage'])  ## first sp

    # df_riv = pd.DataFrame(data=stage, columns=[f'Stage_test_sp{sp}'])
    df_riv = pd.DataFrame(data = riv[sp]['stage'], columns=[f'Stage_test_sp{sp}'])
    
    df_riv.plot(figsize=(12,8))
    df_riv.to_csv(os.path.join(odir, f'df_riv_check_sp{sp}.csv'))
...
if __name__ == "__main__":   
    ...
    # full path to flow model files
    sce = 'flow_2014_2022'
    flow_model_ws = os.path.join(work_dir, 'model_files', sce)
    odir = os.path.join(work_dir, 'checking_mfiles', 'riv', sce)
    ...
    # Readin model files
    ml = read_flow_files("100hr3.nam", flow_model_ws)  # DHmodel_2014to2020.nam  #"100hr3_2023to2125.nam"
