...
def plot_individual_WL_well(AWLN):
    # Plot WL from AWLN wells
    list_AWLN_wells = AWLN['NAME'].unique()
    for well in list_AWLN_wells:
        print(f'Processing well: {well}\n')
        # well = '199-D5-134'
        df = AWLN[AWLN['NAME']==well]        
        ...
        ax.set_ylim([112,121])
        ax.grid()
        plt.savefig(os.path.join('output','water_level_plots','individual_well', f'{well}.png'), dpi=300)
        plt.close()
...
if __name__ == "__main__":
    ...
    WL_obs_noDups['EVENT'] = pd.to_datetime(WL_obs_noDups['EVENT'])
    WL_obs_noDups = WL_obs_noDups[WL_obs_noDups['EVENT'] > dt.datetime(2010,12,31)]
    WL_obs_noDups.to_csv(os.path.join(wldir, "obs_2014_Oct2023", "measured_WLs_all_wells_bk.csv"), index=False)
    
    # Resampled to monthly
    #WL_obs_monthly = resample_to_monthly(wells, WL_obs_noDups)
    WL_obs_noDups = WL_obs_noDups.set_index(WL_obs_noDups['EVENT'])
    
    WL_obs_monthly = WL_obs_noDups.groupby('NAME').resample('M').mean()
    WL_obs_monthly= WL_obs_monthly.reset_index()    
    ...
    WL_obs_monthly = pd.merge(WL_obs_monthly,dfcoords, how='left', on=['NAME'])
...
#    WL_obs_noDups = pd.merge(WL_obs_noDups,dfcoords, how='left', on=['NAME'])
    ...
    WL_obs_monthly['Year'] = WL_obs_monthly['EVENT'].dt.year
    WL_obs_monthly['Month'] = WL_obs_monthly['EVENT'].dt.month
    ...
    #WL_obs_noDups.to_csv(os.path.join(wldir, "obs_2014_Oct2023", "measured_WLs_all_wells.csv"), index=False)
    WL_obs_monthly.to_csv(os.path.join(wldir, "obs_2014_Oct2023", "measured_WLs_monthly_all_wells.csv"), index=False)
    # Save to the format for water level mapping using the R-script
    #col_rename = {''}
    WL_obs_monthly2 = WL_obs_monthly.copy()
    WL_obs_monthly2 = WL_obs_monthly2[WL_obs_monthly2['Year']>2022] # Get CY 2023 data only

    WL_obs_monthly2 = WL_obs_monthly[['NAME','XCOORDS','YCOORDS']]
    WL_obs_monthly2['EVENT'] = WL_obs_monthly['Month']
    WL_obs_monthly2['VAL'] = WL_obs_monthly['VAL_FINAL']
    #WL_obs_monthly2['TYPE'] = 'XD'
    
    WL_obs_monthly2.to_csv(os.path.join(wldir, "obs_2014_Oct2023", "measured_WLs_monthly_all_wells_for_wl_mapping.csv"), index=False)
