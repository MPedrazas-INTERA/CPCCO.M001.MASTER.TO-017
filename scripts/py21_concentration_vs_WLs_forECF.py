...
def plot_WL_vs_conc(wl_meas, crvi_meas_2014, crvi_meas_2021, wl_df, wl_df_2014, conc_df, plot_calib_model = False):
    ...
    for well in wells['NAME']: #["199-H3-10"]:
        ...
        ###HEADS
        ax.scatter(wl1.index, wl1['Water Level (m)'], label='Observed WL', c="navy", s=20, zorder=5)
        ...
        if date_range == "full": #set x-limit
            ...
            if well == "199-H4-85":
                ax2.text(pd.to_datetime('2022-09-25'), text_y, 'Rebound Study Period', ha='right', va='top', rotation=90, color='k', alpha=0.75, path_effects=[pe.withStroke(linewidth=4, foreground='white')], zorder=10)
            else:
                ax2.text(pd.to_datetime('2022-12-5'), text_y, 'Rebound Study Period', ha='right', va='top', rotation=90, color='k', alpha=0.75, path_effects=[pe.withStroke(linewidth=4, foreground='white')], zorder=10)
            ...
            ax2.text(pd.to_datetime('2022-10-25'), text_y, 'Rebound Study Period', ha='right', va='top', rotation=90, color='k', alpha=0.75, path_effects=[pe.withStroke(linewidth=4, foreground='white')], zorder=10)
    ...
    for well in wells.NAME.unique(): #["199-H4-84"]: # #["199-H4-84"]:#:
        ...
        try:
            ax.plot(plot_rebound['Observed'], plot_rebound['Residual'], 'o', markerfacecolor="steelblue", markeredgecolor='blue',
                    markeredgewidth=1, markersize=8, alpha=0.5, zorder = 2, label="Rebound Study")
        except:
            pass
        try:
        ...
        ax.set_xlabel('Observed Water-Level (m)', fontweight='bold', fontsize=14)
        ax.set_xlim([112.8, 118.5])
        ax.set_ylim([-3,3])
