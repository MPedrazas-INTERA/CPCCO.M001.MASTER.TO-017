...
def assign_pmp_rates(ifile, sce, ofile):
    ...
    rt2.loc[flag, f'{sce}'] = -rt.loc[flag,'max1422']
    ...
    rt2.loc[flag,f'{sce}'] = rt.loc[flag,'avg_cy2022']

    # If sce1_flag = stop -> stop
    flag = rt[f'{sce}_flag'] == 'stop'
    rt2.loc[flag,f'{sce}'] = 0

    # If sce1_flag = man (manually defined by user)
    flag = rt[f'{sce}_flag'] == 'man'
    rt2.loc[flag,f'{sce}'] = rt2.loc[flag,f'{sce}_val']

    ###Same for {sce}a: #MP addition
    rt2.loc[:,f'{sce}a'] = 0
    flag = (rt[f'{sce}a_fla'] == 'max') & (rt['Type'] == 'E')
    rt2.loc[flag,f'{sce}a'] = -rt.loc[flag,'max1422']
    flag = (rt[f'{sce}a_fla'] == 'max') & (rt['Type'] == 'I')
    rt2.loc[flag,f'{sce}a'] = rt.loc[flag,'max1422']
    flag = (rt[f'{sce}a_fla'] == 'avg')
    rt2.loc[flag,f'{sce}a'] = rt.loc[flag,'avg_cy2022']
    flag = rt[f'{sce}a_fla'] == 'stop'
    rt2.loc[flag,f'{sce}a'] = 0
    flag = rt[f'{sce}a_fla'] == 'man'
    rt2.loc[flag,f'{sce}a'] = rt2.loc[flag,f'{sce}a_val']
    ...
    # Choose final wells
    rt2 = rt2.loc[rt2['Active'] == 1]
    rt2 = rt2.loc[(rt2['Type'] == 'E') | (rt2['Type'] == 'I')]
    rt2 = rt2.loc[(rt2[f'{sce}_flag'] != 'stop')]

    # E = rt2.loc[(rt2['Type'] == 'E') & (rt2['System'] == 'DX')]
    # I = rt2.loc[(rt2['Type'] == 'I') & (rt2['System'] == 'DX') & (~rt2['ID'].str.startswith("SF"))]

    # Total injection extraction rate:
    selected_cols = ['ID', 'XCOORDS', 'YCOORDS', 'Type', 'System', 'SP109_2023', 'SP110_2023', \
                     'avg1422', 'avg2122', 'max1422', f'{sce}_flag', f'{sce}', f'{sce}a_fla', f'{sce}a']
    rt4 = rt2.loc[:,selected_cols]
...
def gen_well_info_csv_file(df_new_rate,sce):
    ...
    # Create a duplicate of 100HR3\model_packages\pred_2023_2125\mnw2
...
def gen_mnw2(rt2, sce, opt_run_allocateqwell):
    ...
    for i in range(1,sp_pmp1+1,1): # assign rate Jan2023 to Dec2032
        rt3.loc[:,i] = rt2.loc[:,f'{sce}']*gpmtom3d
    ...
    if soil_flushing:
            ...
            if well.startswith("SF"):
                for i in range(1,nsp+1,1):  # assign rates for soil flushing
                    if i not in soil_flushing_range:
                        rt3.loc[rt3.ID == well, i] = 0 #soil-flushing wells, assign zero outside of soil flushing range.
                    if i in soil_flushing_range:
                        rt3.loc[rt3.ID == well, i] = rt2[f"{sce}"].loc[rt2.ID == well].iloc[0]*gpmtom3d #assign rate during soil flushing range
        ...
        for well in turnoffWells.ID.unique():
            if (well != '199-D5-128_I_DX') and (well != '699-95-48B_I_DX'):   ##simply to balance inj - ext during soil flushing
                for i in range(1, sp_pmp1+1, 1):  #assign 0 as rate during soil flushing
                    if i in soil_flushing_range:
...
def gen_mnw2a(rt2, sce, opt_run_allocateqwell):
    ...
    for i in range(sp_pmp1+1,sp_pmp2+1,1): # assign rate Jan2032 to Dec2040
        rt3.loc[flag,i] = rt2.loc[:,f'{sce}a']*gpmtom3d
        #print(i)
    ...
    if soil_flushing:
        ...
        turnoffWells = rt3.loc[~rt3.ID.str.startswith("SF") & (rt3.System == "DX") & (rt3.Type == "I")]
        for well in turnoffWells.ID.unique():
            # if well != '199-D5-148_I_DX':
            if (well != '199-D5-128_I_DX') and (well != '699-95-48B_I_DX'):   ##simply to balance inj - ext during soil flushing
                for i in range(1, sp_pmp1+1, 1):  #assign 0 as rate during soil flushing
                    if i in soil_flushing_range:
                        rt3[i].loc[rt3.ID == well] = 0
            if (well != '699-95-48B_I_DX'):  ##simply to balance inj - ext during soil flushing
                for i in range(sp_pmp1 + 1, sp_pmp2 + 1, 1):  # assign 0 as rate during soil flushing
                    if i in soil_flushing_range:
                        rt3[i].loc[rt3.ID == well] = 0
        sum = rt3.sum()
    ...
    # Save pmp rate to file
    # dst2 = os.path.join(wdir,'model_packages','pred_2023_2125',f'mnw2_{sce}a')
...
if __name__ == "__main__":
    ...
    ofile = os.path.join(wdir, 'gis', 'shp', 'wellrate_added_sce10_pp.shp')
