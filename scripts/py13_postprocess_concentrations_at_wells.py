...
def process_ucn_file(ucnfile, all_lays = True):
...
def plot_concentrations(df_conc, chemdata):

    for well in df_conc['NAME'].unique():
        fig,ax = plt.subplots()
        for lay in range(1, nlay+1):
            toplot = df_conc[df_conc['NAME'] == well][df_conc['Layer'] == lay]
            ax.plot(range(len(toplot)), toplot['Conc'], label = f'model layer {lay}')
            ax.legend()
        plt.grid(True)
        plt.title(f'{well}')
...
if __name__ == "__main__":
    ...
    # plot_concentrations(df_conc, chemdata)