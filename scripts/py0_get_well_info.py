...
def get_wells_ij(wellcsv, rates):
    print("Getting row and column info for each well")
    wells = pd.read_csv(wellcsv, index_col = 0, skiprows = [0,2])
    
    wellrates = pd.read_csv(rates, index_col=0)
    wellrates.dropna(axis=0, inplace=True) #drop any NaN rows
    if type == 'extractionwells':                
        wellrates2 = wellrates.loc[(wellrates != 0).any(axis=1)]     ## drop extraction wells where pumping = 0
        ext = wellrates2.loc[wellrates2.index.str.contains("_E_")]       ## isolate extraction wells
        #print(ext.index)
        wells = wells.loc[ext.index]
    else: #allwells
        pass
    ...
    wells_gdf = gpd.GeoDataFrame(wells,
                                 geometry=gpd.points_from_xy(wells['XW'], wells['YW']),
                                 crs = mycrs)

    gridwells = gpd.sjoin(grid, wells_gdf, how='right')
    df = gridwells.loc[:,['XW', 'YW', 'I', 'J', 'Ztop', 'Zbot']]
    df = df[~df.index.duplicated(keep='first')]

    df.columns = ['X', 'Y', 'Row', 'Col', 'Ztop', 'Zbot']    
