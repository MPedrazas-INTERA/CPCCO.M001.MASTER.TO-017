...
if __name__ == "__main__":
    ...
    ifile_path2 = os.path.join(cwd, 'RiverCSV', '03_River_Jan2023toOct2023.csv') #until Oct 2023
    df_2023 = pd.read_csv(ifile_path2) #1,032,083 rows
    df_2023.dropna(inplace=True) #979,374
    df_2023["SP"] = df_2023["SP"] - 96  # correcting SPs to it starts with SP 1, not SP 97
    df_2023 = df_2023.loc[(df_2023.SP >= (109)) & (df_2023.SP <= 118)] #only need SP 109 to 118.
    print("Correct SPs: ", df_2023.SP.unique())
    ...
    ###Concatenate from SP 1 in Jan 2014 to Oct 2023 (SP 118):
    keep_cols = df_2014_2022.columns
    df_final = pd.concat([df_2014_2022, df_2023[keep_cols]], axis=0)
    ofile_path = os.path.join(cwd, 'RiverCSV','04_River_2014_Oct2023_correctK_v2.csv')
