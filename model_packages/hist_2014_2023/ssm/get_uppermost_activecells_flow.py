...
def get_BtmsDF(path2flow, df, all_lays):
    ...
    exe = 'mf2k-mst-cpcc09dpl'
    ws = path2flow
    m = flopy.modflow.Modflow.load("100hr3.nam", verbose=False, model_ws=ws, exe_name=exe, check=False,
...
if __name__ == "__main__":
    ...
    if cluster:
        outDir = cwd
        inputDir = cwd
        path2flow = os.path.join('..', '..', '..', 'mruns', 'calib_2014_2023', 'flow_2014_2023_rr2')
    else: #OneDrive
    ...
    path2hds = os.path.join(path2flow, '100hr3.hds')
    print("Using HDS file here: ", path2flow) 
    ...
    if target == 'src_cells':
        ...
        if cluster:
            ...
            df_zon = pd.read_csv("cr6_source_zones.dat", delim_whitespace=True) ## add to transport folder
    ...
    print(f"This model has {ntimes} SPs")
    ...
    if target == 'src_cells':
        ...
        if modify_ssm: #modifying SSSM package to update with active layer for each SP for each source cell.
            fin = "100HR3_2014_2023_transport_stomp2ssm.ssm" #"100HR3_2014_2020_transport_stomp2ssm.ssm"
            fout = "100HR3_2014_2023_transport_stomp2ssm_activLays.ssm"
