...
def read_ssm_resampled():
    ...
    result_files = [x for x in result_files if x != []]  ## CURRENTLY 100-D-100-unexv_1 is empty!
    ...
    for file in result_files:
        # print(file)
        rrs[file] = pd.read_csv(file)
    newkeys = []
    for k in rrs.keys():
        newkeys.append(k[len(str(ws))+1:-14])   ##isolate bore names directly from ws name. Depends directly on length of directory name!
    rrs2 = dict(zip(newkeys, rrs.values()))     ##change dict keys to bore names.
    rrs3 = dict([(k,v) for k, v in rrs2.items() if k in z.keys()]) ##isolate to bores only within z (weight > 0)
    ...
    for bore, mult in z.items():
        print(mult[0])
        rrs3[bore]['rr'] = (rrs3[bore]['sum/dt'] * 365.25 / 1000000) / mult[0]  ##Daily to annual, ug to g.

    return rrs, rrs2, rrs3    ## return all 3 for QA, rrs3 is final result for plotting

### READ WATER TABLE DATA

def read_wt_data(rrs3, sps):
    ...
    for file in wt_files:
        ...
        wts[file] = pd.read_csv(file).iloc[:len(sps)]

    wtkeys = []
    for k in wts.keys():
        wtkeys.append(k[len(str(wtpath)) + 9:-4])  ## +9 gets rid of /make_BC_
    wts2 = dict(zip(wtkeys, wts.values()))


    ## 3. APPEND WT HEADS TO RRS2 DFs
    for bore in list(rrs3.keys()):
        for wt in list(wts2.keys()):
            print(wt)
            if wt in bore:
                rrs3[bore]['Head'] = wts2[wt]['Head']

    return wts2

#### 4. PLOTTING
def plot_release_rates(rrs3, wts2):
    """
    ...
    title_dict = dict(zip(rrs3.keys(), sites))      ## make sure key-val pairs match as expected
    ...
    x = pd.to_datetime(wts2['100-D-100']['Date']) ##snag date column from any of the wts2 dataframes as x axis
    ...
    for bore in rrs3.keys():
        if rrs3[bore]['rr'].all() > 0:          ##do not plot when rr = 0 over entire timeseries
            mean = [rrs3[bore]['Head'].mean()] * len(rrs3[bore]['Head'])
            fig, ax = plt.subplots(figsize = (10,6))
            # ax.set_title(f'Cr(VI) at {bore}')
            ax.set_title(f'Cr(VI) at {title_dict[bore]}')
            ax.plot(x, rrs3[bore]['rr'], color = 'xkcd:crimson', label  = 'Release Rate')
            # ax.plot(rrs3[bore], rrs3[bore]['rr'], color='red', label='Release Rate')
            ax.grid(True)
            ax.set_xlabel('Year')
            ax.set_ylim(0, rrs3[bore]['rr'].max()*1.3)      ##axis limits a function of individual RR curves
            ax.set_xlim(xmin,xmax)
            ax.set_ylabel('Release Rate (kg/year)')

            ax2 = ax.twinx()
            ax2.plot(x, rrs3[bore]['Head'], label = 'Water Table')
            ax2.plot(x, mean, '--', color='purple', linewidth=2, label = 'Average Water Table')
            ax2.set_ylabel('Water Table Elevation (m)')
            ax2.set_ylim(rrs3[bore]['Head'].min()*0.98, rrs3[bore]['Head'].max()*1.01) ##axis limits function of specific WT location
            ...
            plt.tight_layout()
            # plt.show()
            # plt.savefig(os.path.join(outputDir, f'{bore}_2014_2023_weights_fullname.png'))
...
if __name__ == '__main__':
    ...
    rrs, rrs2, rrs3 = read_ssm_resampled()

    wts2 = read_wt_data(rrs3, sps)

    plot_release_rates(rrs3, wts2)
